...
- `/` 以下が近代的な実装になる：jQuery/SoundJS 廃止、スマホ対応、白基調＋ピンクの本家寄りデザイン、テスト、CI、キャッシュ、広告なし
...
**デザイン方針（2026-09-11 変更）**：ダークモードは採用しない。はてなブックマークの
パロディサイトなので、本家はてなブックマークと同じ**白基調**にし、本家の青系アクセント
に対してこちらは**ピンク**を使う（＝クラシック版の配色 #FFC8C8 / #FF9999 に近い方向）。
ダークパターン（同意誘導・偽の緊急性・隠しコストなど欺瞞的な UI）は使わない。
ダーク**テーマ**の実装（`prefers-color-scheme` 対応・トグル）自体を行わない。

- 配色：背景は白〜ごく薄いグレー（本家はてブ準拠）、本文・罫線はグレー系、アクセントに
  ピンク（リンクのホバー、ボタン、殴るチェックの強調、カード罫線など）。クラシックの
  7 色ランダム罫線は廃止し、ピンクの濃淡 1〜2 階調に統一（本家のカード然とした見た目に寄せる）
...
- OGP / `theme-color`（ピンク系）/ favicon の SVG 化（URL はドメイン確定後に差し込む）
...
- [ ] ブコメ一覧（entry.erb）の見た目調整。`_dev/mock/`（gitignore対象）にダミーデータの
  静的HTML・CSS・JSを置いて試行錯誤中。固まるまで `public/app.css` 等の本番ファイルには反映しない
- [ ] 未定（着手時に決める）
